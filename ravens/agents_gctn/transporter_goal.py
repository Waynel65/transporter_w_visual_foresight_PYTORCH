...
from ravens.models_gctn import Attention, Transport, TransportGoal
...
class TransporterAgent:
    ...
    def get_top_k_acts(self, k, input_image, goal_image):
        """Run inference and return top k actions given visual observations.
        ...
        if k > 1:
            ...
            max_map = np.max(transport, axis=2)
            max_map_max = np.max(max_map)
            max_map[max_map * 100 < max_map_max] = 0.0
            max_map_valid = max_map > 0
            n = min(n, np.sum(max_map_valid))
            top_n = np.argsort(max_map.flatten())[::-1][:n]
            top_n_pixels = []

            # Select the top n pixels.
            for i in range(n):
                top_n_pixels.append(np.unravel_index(top_n[i], shape=max_map.shape))
            
            # K-means cluster.
            km = KMeans(n_clusters=k).fit(np.array(top_n_pixels))
            centers = km.cluster_centers_
            labels = km.labels_
            
            # Select the indices for the k-cluster.
            best_k_pixels = []
            top_n_k_maps = []
            for k_idx in range(k):
                top_n_k_map = np.zeros(max_map.shape)
                top_n_k_indices = np.argwhere(labels==k_idx).flatten()
                n_indices = top_n_k_indices.shape[0]
                for top_n_k_idx in range(n_indices):
                    pixel_0 = top_n_pixels[top_n_k_indices[top_n_k_idx]][0]
                    pixel_1 = top_n_pixels[top_n_k_indices[top_n_k_idx]][1]
                    top_n_k_map[pixel_0, pixel_1] = max_map[pixel_0, pixel_1]
                top_n_k_maps.append(top_n_k_map)
                best_k_pixel = np.unravel_index(np.argmax(top_n_k_map.flatten()), shape=max_map.shape)
                best_k_pixels.append(best_k_pixel)
            
            # Retrieve the angle index.
            best_k_pixels_with_angle = []
            for best_k_pixel in best_k_pixels:
                angle_idx = max_map_idx[best_k_pixel[0], best_k_pixel[1]]
                best_k_pixels_with_angle.append(
                    np.array([best_k_pixel[0], best_k_pixel[1], angle_idx]))
        else:
            best_k_pixels_with_angle = []
            ...
            best_k_pixels_with_angle.append([p1_pixel[0], p1_pixel[1], p1_theta])

        acts = []
        for i in range(k):
            p1_pixel = best_k_pixels_with_angle[i][:2]
            p1_theta = best_k_pixels_with_angle[i][2] * (2 * np.pi / transport.shape[2])
            ...
            act = {'pose0': (p0_position, p0_rotation),
    ...
    def load_from_path(self, attention_path, transport_path, num_iter):
        """Load pre-trained models."""

        self.attention_model.load(attention_path)
        self.transport_model.load(transport_path)
        self.total_iter = num_iter

        print(f'Load attention model: {attention_path}')
        print(f'Load transport model: {transport_path}')
        print(f'n_iter: {num_iter}')
