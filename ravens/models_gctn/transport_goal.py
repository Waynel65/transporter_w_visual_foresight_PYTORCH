...
class TransportGoal:
    ...
    def forward(self, in_img, goal_img, p, apply_softmax=True):
        ...
        assert in_img.shape == goal_img.shape, f'{in_img.shape}, {goal_img.shape}' # (160,160,6)

        # input image --> Torch tensor
        input_unproc = np.pad(in_img, self.padding, mode='constant')    
        input_data = self.preprocess(input_unproc.copy())               # (224,224,6)
        ...
        goal_data = self.preprocess(goal_unproc.copy())                 # (224,224,6)
        ...
        # pivot = np.array([p[1], p[0]]) + self.pad_size # p is generated by attention module during inference
        
        pdb.set_trace()

        # ! hardcoded for testing
        pivot = np.array(162,65) + self.pad_size
        rvecs = self.get_se2(self.num_rotations, pivot) # rotation vector of shape (36,8)
        print(f"[TRANS_goal] RVECS have a shape of {rvecs.shape}") 

        # pytorch convention start
        # ? any problem with combining 3 resnets into 1?
        # in_logits, kernel_nocrop_logits, goal_logits = self.model(in_tensor, goal_tensor) # all have shape of (1,3,224,224)

        # ! hardcoded output from the model
        dummy_in_logits = torch.full((1,3,224,224), 1.0) # filled with ones
        dummy_kernel_nocrop_logits = torch.full((1,3,224,224), 2.0) # filled with twos
        dummy_goal_logits = torch.full((1,3,224,224), 3.0) # filled with threes
        ...
        goal_x_kernel_logits = goal_logits * kernel_nocrop_logits   # (1,3,224,224)

        # Crop the kernel_logits about the picking point and get rotations.
        crop = goal_x_kernel_logits.clone()                                 # (1,3,224,224)
        crop = crop.repeat(self.num_rotations, 1, 1, 1)                     # (36,3,224,224)
        ...
        crop = rotated_crop # (36,3,224,224)
